...
class FormBBlock(object):
    def __init__(self, phys_target, phys_range, new=None, mixed_new=None,
        ...
        self._diag_Ah = [[False]*c for x in range(r)]
        ...
        for i, j in product(range(self.shape[0]), range(self.shape[1])):
            ...
            for p in projs:
                ...
                if form is None:
                    ...
                    #dprint1(i, j,
                    #      self.target_phys_list[i], self.range_phys_list[j],
    ...
    def set_mat_from_blk(self, i, j,  Ah, mblk):
        # this returns block matrix generated by
        self._diag_Ah[i][j] = Ah

        size = int(np.sqrt(len(mblk)))
        k = 0
        for ii in range(size):
            for jj in range(size):
                ...
                self.set_matvec(i+ii, j+jj, 1, mblk[k])
                k = k + 1
