...
class RealImagCoefficientGen(ABC):
    ...
    def __radd__(self, other):
        ...
        if isinstance(other, NumbaCoefficient):
            if numba_debug:
                dprint1("in __radd__: NumbaCoefficient is converted to PyComplexCoefficient")
            obj = complex_coefficient_from_real_and_imag(other.real, other.imag)
            return obj + self
        else:
            return NotImplemented
        
    def __mul__(self, scale):
        assert not isinstance(scale, RealImagCoefficientGen), "multiplication is not defined"
...
class CCBase(RealImagCoefficientGen):
    def __init__(self, kind, vdim=None, height=None, width=None):
        RealImagCoefficientGen.__init__(self, kind, vdim=vdim, height=height, width=width)
